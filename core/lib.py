...
def booking_overlaps(booking):
    bookings = Booking.objects.filter(room=Room.objects.get(
        id=booking['room_id']), date=booking['date'])
    new_start_time = timezone.datetime.strptime(
        booking["start_time"], "%H:%M").time()
    new_end_time = timezone.datetime.strptime(
        booking["end_time"], "%H:%M").time()
...
def create_booking(booking):
    ...
    for field in ["organiser", "title", "details"]:
        if booking[field] == "":
            errors.append(ValidationError(
                f"{field} cannot be empty", code=f"empty_{field}"))

    # Check that the values aren't too long
    for field in ["organiser", "title"]:
        if len(booking[field]) > 100:
            errors.append(ValidationError(
                f"{field} cannot be longer than 100 characters", code=f"{field}_too_long"))
    ...
    if timezone.datetime.strptime(booking["start_time"], "%H:%M").time() < timezone.datetime.strptime("09:00", "%H:%M").time():
        errors.append(ValidationError("Booking start time is outside of opening hours: 9:00 - 17:00",
    ...
    if timezone.datetime.strptime(booking["end_time"], "%H:%M").time() > timezone.datetime.strptime("17:00", "%H:%M").time():
        errors.append(ValidationError("Booking end time is outside of opening hours: 9:00 - 17:00",
